...
const TierPulse3D = dynamic(() => import("./TierPulse3D"), {
  ssr: false,
      ...
      {tiers.map((tier) => {
        ...
        return (
          <div key={tier} className="flex flex-col items-center gap-1 flex-1">
            <div className="w-full rounded-sm transition-all duration-700" style={{ height: `${Math.max(4, pct)}%` }}>
...
export function TierPulseLazy({ counts }: Props) {
  ...
  if (!show3D) return <TierPulseFallback counts={counts} />;
  return <TierPulse3D counts={counts} />;
}
